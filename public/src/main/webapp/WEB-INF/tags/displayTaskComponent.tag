...
<div id="text-task-${id}-edit" hidden="true" name="display-task-div">
    <form class="update" action="/dashboard" method="POST" id="update-task-${id}-form">
        ...
        <p>Description <textarea id="description_${id}" cols="40" rows="5" onblur="validateUpdateBean(['description'], ${id});"><c:choose><c:when test="${s:hasTaskBean(mttSession, id)}"> value="${s:getTaskBean(mttSession, id).description}"></c:when><c:otherwise>${description}</c:otherwise></c:choose></textarea></p>
        ...
        <input type="submit" class="update-button" id="update-task-button" name="update-task" value="Update" onmouseover="validateUpdateBean(['title','description'],${id});"/>
    </form>
